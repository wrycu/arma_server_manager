...
def server_start(schedule_id: int = 0) -> None:
    ...
    try:
        command.extend([f"-serverMod={x}" for x in [y['mod']['filename'] for y in server_details['collection']['mods'] if y['mod']['server_mod']]])
        command.extend([f"-mod={x}" for x in [y['mod']['filename'] for y in server_details['collection']['mods'] if not y['mod']['server_mod']]])
    except KeyError:
