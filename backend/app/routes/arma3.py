...
def get_mod_overview(mod_id: int) -> tuple[dict[str, str], int]:
    ...
    """
    return {
        'results': current_app.config['MOD_MANAGERS']['ARMA3'].steam_api.get_mod_details(mod_id),
...
def get_mod_subscriptions() -> tuple[dict[str, str], int]:
    ...
    """
    try:
        return current_app.config['MOD_MANAGERS']['ARMA3'].get_subscribed_mods(), HTTPStatus.OK
...
def add_mod_subscription() -> tuple[dict[str, str], int]:
    ...
    try:
        for mod in request.json['mods']:
            current_app.config['MOD_MANAGERS']['ARMA3'].add_subscribed_mod(mod['steam_id'])
    except Exception as e:
        return {
            'message': str(e),
        }, HTTPStatus.BAD_REQUEST
...
def get_mod_subscription_details(mod_id: int) -> tuple[dict[str, str], int]:
    ...
    try:
        return {
            'results': current_app.config['MOD_MANAGERS']['ARMA3'].get_subscribed_mod_details(mod_id),
            'message': 'Retrieved successfully',
        }, HTTPStatus.OK
...
def update_mod_subscription_details(mod_id: int) -> tuple[dict[str, str], int]:
    ...
    """
    try:
        current_app.config['MOD_MANAGERS']['ARMA3'].update_subscribed_mod(mod_id, request.json)
        return {
...
def get_mod_subscription_image(mod_id: int) -> tuple[Response, int] | tuple[dict[str, str], int]:
    """
    ...
    """
    try:
        image = current_app.config['MOD_MANAGERS']['ARMA3'].get_subscribed_mod_image(mod_id)
        return Response(
            image['image_data'],
